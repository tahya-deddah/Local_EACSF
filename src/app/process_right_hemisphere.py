...
def optimize_csfdensity (surface):
	...
	EACSFFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density.txt")
	EACSFMaxValueFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_InterpolationMaxValue_Directory", "RH_" + surface + "_CSF_Density.txt")
	EACSFMinValueFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_InterpolationMinValue_Directory", "RH_" + surface + "_CSF_Density.txt") 
	EACSFInterpolatedFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density_Interpolated.txt")	
	EACSFFinalFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density_Final.txt")
	AbsoluteDifferenceFile = os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Original_Interpolated_Absolute_Difference.txt" )
	...
	"--ScalarsFile", "RH_" + surface + "_CSF_Density.txt", "--Scalars_Name", 'CSF_Density_Original'])
	call_and_print([args.AddScalarstoPolyData, "--InputFile", "RH_" + surface + "_CSF_Density.vtk", "--OutputFile", "RH_" + surface + "_CSF_Density.vtk",\
	"--ScalarsFile", "RH_" + surface + "_CSF_Density_Interpolated.txt", "--Scalars_Name", 'CSF_Density_Interpolated'])
	...
	os.rename(EACSFFile, os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density_Original.txt"))	
	call_and_print([args.AddScalarstoPolyData, "--InputFile", "RH_" + surface + "_CSF_Density.vtk", "--OutputFile", "RH_" + surface + "_CSF_Density.vtk",\
		"--ScalarsFile", "RH_" + surface + "_CSF_Density_Final.txt", "--Scalars_Name", 'CSF_Density_Final'])
...
def main_loop(args):
	...
	CSFDensity_Path = os.path.join(args.Output_Directory, "LocalEACSF", "RH_Directory","RH_" + surface + "_CSF_Density_Final.txt")
	if (os.path.isfile(CSFDensity_Path)):
		...
		if(args.NotInterpolated):
			...
			os.rename(os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density.txt"),\
				os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory", "RH_" + surface + "_CSF_Density_Final.txt"))	

		os.chdir(os.path.join( args.Output_Directory, "LocalEACSF", "RH_Directory"))
		if(args.Smooth) :
			call_and_print([args.HeatKernelSmoothing , "--InputSurface", "RH_" + surface + "_CSF_Density.vtk", "--NumberIter", "@NumberIter@", "--sigma"\
				, "@Bandwith@", "--OutputSurface", "RH_" + surface + "_CSF_Density.vtk"])
...
def processing(args, DirectoryName, Surface, ImageDimension):
	...
	if StreamlinesExistenceTest ==True :
		...
		if(args.Use_75P_Surface):
			...
			call_and_print([ComputeAverageMesh, "--inputFilename1", "RH_MID.vtk", "--inputFilename2", "RH_GM.vtk", "--outputFilename", "RH_75P.vtk"])
	...
	CSFDensdityTxtFile = os.path.join(Directory,"RH_" + Surface + "_CSF_Density.txt")  
